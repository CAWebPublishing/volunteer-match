...
if ( ! class_exists( 'Volunteer_Match_Plugin_Update' ) ) {
	...
	class Volunteer_Match_Plugin_Update {
		 ...
		 */
		protected $transient_name = 'volunteer_match_update_plugins';
		 ...
		 */
		protected $repo = 'https://api.github.com/repos/Danny-Guzman/volunteer-match';
		...
		public function __construct( $plugin_slug ) {
			...
			add_filter( 'pre_set_site_transient_update_plugins', array( $this, 'volunteer_match_check_plugin_update' ) );

			// Define the alternative response for information checking.
			add_filter( 'site_transient_update_plugins', array( $this, 'volunteer_match_add_plugins_to_update_notification' ) );

			add_filter( 'plugins_api', array( $this, 'volunteer_match_update_plugins_changelog' ), 20, 3 );
			...
			add_filter( 'upgrader_source_selection', array( $this, 'volunteer_match_upgrader_source_selection' ), 10, 4 );
		 ...
		 * @return array
		 */
		public function volunteer_match_check_plugin_update( $update_transient ) {
		 ...
		 * @return array
		 */
		public function volunteer_match_add_plugins_to_update_notification( $update_transient ) {
			$volunteer_match_update_plugins = get_site_transient( $this->transient_name );
			if ( ! is_object( $volunteer_match_update_plugins ) || ! isset( $volunteer_match_update_plugins->response ) ) {
				return $update_transient;
			}
			...
			$update_transient->response = array_merge(
				...
				$volunteer_match_update_plugins->response
			);
		 ...
		 */
		public function volunteer_match_update_plugins_changelog( $result, $action, $args ) {
			if ( isset( $args->slug ) && $args->slug === $this->slug ) {
				$volunteer_match_update_plugins = get_site_transient( $this->transient_name );
				if ( isset( $volunteer_match_update_plugins->response ) && isset( $volunteer_match_update_plugins->response[ $this->plugin_file ] ) ) {
					$tmp = $this->plugin_details();

					$tmp['version']      = $volunteer_match_update_plugins->response[ $this->plugin_file ]->new_version;
					$tmp['last_updated'] = $volunteer_match_update_plugins->response[ $this->plugin_file ]->published_date;

					$tmp['sections']['Changelog'] = $this->volunteer_match_get_plugin_changelog( $tmp['version'] );
		 ...
		 * @return string
		 */
		public function volunteer_match_get_plugin_changelog( $ver = 'master' ) {
		 ...
		 * @return string
		 */
		public function volunteer_match_upgrader_source_selection( $src, $rm_src, $upgr, $options ) {
...
new Volunteer_Match_Plugin_Update( plugin_basename( plugin_dir_path( __DIR__ ) ) );
